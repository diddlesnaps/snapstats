...
export default async (message: functions.pubsub.Message): Promise<void> => {
  try {
    ...
    await ArchitecturesModel.insertMany(message.json.map((architecture) =>
        (architecture.name) ? architecture : {...architecture, name: "unset"}
    ));
  } catch (err) {
    console.error("collectors/collectStats.js: " +
        `Error: architectures: ${err.toString()}`);
  }
};
